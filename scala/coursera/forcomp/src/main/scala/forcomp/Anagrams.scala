...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    val grouped = lowerCaseWord.groupBy((char: Char) => char)
    val mapped = grouped.map(p => (p._1, p._2.length))
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    if (y.isEmpty) {
      ...
      val (head, tail) = x.span((p: (Char, Int)) => p._1 != y.head._1)
      val amount = tail.head._2 - y.head._2
      val newRest = if (amount > 0) (tail.head._1, amount) :: tail.tail else tail.tail
      subtract(head ::: newRest, y.tail)
    }
  }
    ...
    def loop(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case list =>
        for {
          ...
          remaining <- loop(subtract(occurrences, combinations))
